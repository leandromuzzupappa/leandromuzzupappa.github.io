...
export const Hero = () => {
  ...
  const { contextSafe } = useGSAP({ scope: heroRef, dependencies: [isCollapsed, scroll] });
  ...
  const animateBackground = contextSafe(({ scroll, direction, target, isCollapsed }: IAnimation<HTMLDivElement>) => {
    const endConfig = {
      ['--overlay-opacity']: .6,
      borderRadius: 100,
      height: '50px',
    ...
    if (!isCollapsed) {
      gsap.to(target, {
        ...
        height: `${100 - scroll * .1}vh`,
        maxWidth: '100vw',
        scaleX: Math.max(1 - scroll / 3000, .5),
        y: scroll * 0.05,
        duration: 0.8,
        ease: "power3.out",
      })
  ...
  const animateTexts = contextSafe(({ scroll, direction, targets, isCollapsed }: IAnimation<HTMLDivElement>) => {
    const [, title] = targets!;
    const bgRect = bgRef.current?.getBoundingClientRect();

    const ease = CustomEase.create("custom", "M0,0 C0,0.507 0.073,0.774 0.2,0.9 0.331,1.031 0.504,1 1,1 ");
    const opacity = 1 - scroll / 500;
    const padding = 10;
    const position = opacity < .1 ? 'fixed' : opacity > .2 ? 'relative' : 'fixed'
    ...
    if (!isCollapsed) {
      ...
      gsap.to(targets!, {
        opacity: opacity < .3 ? 0 : opacity > .7 ? 1 : opacity,
        y: -scroll * .5,
        duration: 1.2,
        ease: "power4.out",
        stagger: 0.5,
